...
export default {
  ...
  methods: {
    getChapter() {
      fetch('https://api.quran.com/api/v4/chapters/89?language=id', { method: 'GET', })
        .then(response => {
          if (response.ok) {
    ...
    getAudio() {
      fetch('https://api.quran.com/api/v4/chapter_recitations/3/89', { method: 'GET', })
        .then(response => {
          if (response.ok) {
            return response.json();
          }
    ...
    getVerses(){
      fetch('https://api.quran.com/api/v4/quran/verses/uthmani?chapter_number=89', {
        method: 'GET'
      })
        .then(res =>{
          if(res.ok){
    ...
    getTranslate() {
      fetch('https://api.quran.com/api/v4/quran/translations/33?chapter_number=89', {
        method: 'GET'
      })
        .then(response => {
          if (response.ok) {
