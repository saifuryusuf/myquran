...
export default {
  ...
  methods: {
    getChapter() {
      fetch('https://api.quran.com/api/v4/chapters/81?language=id', { method: 'GET', })
        .then(response => {
          if (response.ok) {
    ...
    getAudio() {
      fetch('https://api.quran.com/api/v4/chapter_recitations/3/81', { method: 'GET', })
        .then(response => {
          if (response.ok) {
            return response.json();
          }
    ...
    getTranslate() {
      fetch('https://api.quran.com/api/v4/quran/translations/33?chapter_number=81', {
        method: 'GET'
      })
        .then(response => {
          if (response.ok) {
